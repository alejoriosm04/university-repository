...
class Patient():
    def __init__(self):
        ...
        self.time1 = round(random.uniform(1, 240), 0)
        if self.status == "Exámenes de Laboratorio":
            self.time2 = round(random.uniform(1, 240), 0)
        else:
            self.time2 = 0
        if self.status == "Tratamiento":
            self.time2 = round(random.uniform(1, 240), 0)
            self.time3 = round(random.uniform(1, 240), 0)
        else:
            self.time3 = 0
        if self.status == "Salida":
            self.time2 = round(random.uniform(1, 240), 0)
            self.time3 = round(random.uniform(1, 240), 0)
            self.time4 = round(random.uniform(1, 240), 0)
        else:
            self.time4 = 0
    ...
    def __gt__(self, nextPatient):
        return self.total_time > nextPatient.total_time
    
    def __lt__(self, nextPatient):
        return self.total_time < nextPatient.total_time
    
    def __eq__(self, nextPatient):
        return self.total_time == nextPatient.total_time
