...
public class Graph {
    ...
    EdgeBellmanFord[] edges;
    VertexBellmanFord[] vertices;
    ...
    public Graph(String filename) {
        try {
            ...
            edges = new EdgeBellmanFord[E];
            vertices = new VertexBellmanFord[V + 1];
            numEdges = 0;
            ...
            for (int i = 0; i < E; i++) {
                ...
                int v = in.nextInt();
                int w = in.nextInt();
                EdgeBellmanFord e = new EdgeBellmanFord(u, v, w);
                addEdge(e);
            }
        } catch (IOException e) {
    ...
    public void initialize(int s) {
        for (int i = 1; i <= V; i++) {
            VertexBellmanFord v = new VertexBellmanFord(i, Integer.MAX_VALUE, null);
            vertices[i] = v;
        }
        vertices[s].d = 0;
    }
    ...
    void relax(EdgeBellmanFord e) {
        VertexBellmanFord u = vertices[e.u];
        VertexBellmanFord v = vertices[e.v];
        int w = e.w;
    ...
    boolean edgeFormsNegativeCycle(EdgeBellmanFord e) {
        VertexBellmanFord u = vertices[e.u];
        VertexBellmanFord v = vertices[e.v];
        int w = e.w;
    ...
    void addEdge(EdgeBellmanFord e) {
        edges[numEdges] = e;
        numEdges++;
    }
