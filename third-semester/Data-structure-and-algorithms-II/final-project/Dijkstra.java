...
public class Dijkstra {
    ...
    LinkedList<EdgeDijkstra>[] graph; // Graph
    int[] dist; // shortest distances to each vertex
    int[] prev; // previous vertex in shortest path
    ArrayList<VertexDijkstra> q; // Queue as an ArrayList
    PriorityQueue<VertexDijkstra> pq; // Queue as a PriorityQueue
    ...
    public void findShortestPaths(int source) {
        ...
        pq.add(new VertexDijkstra(source, 0));

        // while (!q.isEmpty()) {
        // Vertex u = poll(q); // Retrieve vertex with smallest distance

        while (!pq.isEmpty()) {
            VertexDijkstra u = pq.poll();
            ...
            for (EdgeDijkstra v : graph[u.node]) { // Visit all nodes "e" that v conects to
                ...
                if (alt < dist[v.to]) { // Check if e's distance can be improved
                    ...
                    pq.add(new VertexDijkstra(v.to, dist[v.to]));
                }
            }
        }
    }
     ...
     */
    private VertexDijkstra poll(ArrayList<VertexDijkstra> q) {
        if (q.isEmpty()) {
            return null;
        }
        // Find vertex with shortest distance
        VertexDijkstra sVertex = q.get(0);
        int sIndex = 0;
        for (int i = 1; i < q.size(); i++) {
            VertexDijkstra v = q.get(i);
    ...
    private void readFile(String fileName) {
        ...
        ArrayList<LinkedList<EdgeDijkstra>> graph2 = new ArrayList<>();
        try {
            ...
            while (in.hasNextLine()) {
                ...
                LinkedList<EdgeDijkstra> al = new LinkedList<>();
    ...
    private void readFileNewFormat(String fileName) {
        try {
            ...
            for (int i = 0; i < numEdges; i++) {
                ...
                int to = in.nextInt();
                int w = in.nextInt();
                graph[from].add(new EdgeDijkstra(to, w));
            }
        } catch (FileNotFoundException ex) {
            System.out.println("No se encontró el archivo " + fileName);
            System.exit(1);
